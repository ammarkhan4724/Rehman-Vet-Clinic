...
function Header() {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? "py-2" : "py-4"}`}>
      <div className={`mx-3 xl:mx-auto max-w-7xl transition-all duration-500 ${scrolled ? "bg-white/90 backdrop-blur-xl shadow-lg shadow-slate-900/5" : "bg-white/75 backdrop-blur-md"} rounded-full border border-white/60`}>
        <div className="px-4 xl:px-6 py-2.5 flex items-center justify-between gap-2">
          <div className="shrink-0">
            <Logo />
          </div>

          <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1 bg-emerald-50/80 border border-emerald-100/60 rounded-full p-1 shrink-0">
            {NAV.map((n) => (
              <a
                key={n.href}
                href={n.href}
                className="px-2.5 xl:px-3 py-1.5 text-[12.5px] xl:text-[13.5px] font-semibold text-slate-700 hover:text-emerald-800 hover:bg-white rounded-full transition-all whitespace-nowrap"
              >
                {n.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span className="whitespace-nowrap font-bold">+92 311 4899904</span>
            </a>
